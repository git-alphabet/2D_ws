...
# Odin mode2 may internally fall back to SLAM before it publishes map->odom, so
# Nav2 must not wait for map TF before its lifecycle manager starts.
...
# 重定位导航入口：显式使用 odin_map_mode:=2，并保持 Nav2 加载已有 2D 地图。
# odin 初始匹配失败时由 odin 内部回退到 SLAM；成功后由 odin 发布 map->odom。
