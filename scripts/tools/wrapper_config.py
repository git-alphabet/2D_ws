#!/usr/bin/env python3
"""Environment, branch resolution, YAML config, and basic helpers."""
...
def resolve_overlay_setup(ws_dir: Path) -> Path:
    ...
    if env_overlay:
        ...
        if candidate.exists() and (allow_overlay_override or _matches_branch_profile(candidate)):
            return candidate
        print(f"[{wrapper_log_tag()}] Ignore OVERLAY_SETUP='{env_overlay}' (branch={branch_safe}).", file=sys.stderr)
    ...
    if colcon_install_base:
        ...
        if candidate.exists() and (allow_overlay_override or _matches_branch_profile(candidate)):
            return candidate
        print(f"[{wrapper_log_tag()}] Ignore COLCON_INSTALL_BASE='{colcon_install_base}' (branch={branch_safe}).", file=sys.stderr)
...
def build_base_env(cfg: CommonConfig) -> str:
    ...
    branch_cache_roots = [str(cfg.ws_dir / ".buildcache"), str(cfg.ws_dir / "build/.buildcache")]
    workspace_install_roots = [str(cfg.ws_dir / "install"), str(cfg.ws_dir / "build/install")]
    ...
    for var_name in ("LD_LIBRARY_PATH", "AMENT_PREFIX_PATH", "COLCON_PREFIX_PATH", "CMAKE_PREFIX_PATH", "PYTHONPATH"):
    ...
    if home_path is not None and home_path.is_dir() and os.access(home_path, os.W_OK):
        ...
        for candidate in [cfg.ws_dir / "log", cfg.ws_dir, Path("/tmp")]:
    ...
    if ros_home_env:
        ...
        ros_home = None
        for candidate in [cfg.ws_dir / "log/.ros", cfg.ws_dir / ".ros", Path("/tmp") / f"ros_home_{os.getuid()}"]:
    ...
    if odin_driver_source_dir.is_dir():
        parts.append(f"export ODIN_ROS_DRIVER_SOURCE_DIR={shlex.quote(str(odin_driver_source_dir))}")

    if cfg.rcutils_logging_severity:
        parts.append(f"export RCUTILS_LOGGING_SEVERITY={shlex.quote(cfg.rcutils_logging_severity)}")

    for var_name in ("__NV_PRIME_RENDER_OFFLOAD", "__GLX_VENDOR_LIBRARY_NAME", "IGN_GAZEBO_RENDER_ENGINE_SERVER", "IGN_GAZEBO_RENDER_ENGINE_GUI"):
