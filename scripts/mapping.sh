...
controller_plugin="$(python_with_ros_env "$REALITY_PARAMS_FILE" <<'PY'
import sys
...
bt_report="$(python_with_ros_env "$REALITY_PARAMS_FILE" "$SCRIPT_NAME" <<'PY'
import os
import sys
from pathlib import Path
...
try:
	...
	print(f"{prefix} Warning: unable to read behavior tree from {cfg_path}: {exc}")
	sys.exit(0)
...
if not enable_rm_bt:
	...
	print(f"{prefix} Behavior tree disabled (selector='{reason}')")
	sys.exit(0)

resolved_path, exists, origin = _resolve_style(style_value)
status = "found" if exists else "missing"
print(f"{prefix} Behavior tree enabled via YAML ({origin}); style='{style_value}' => {resolved_path} [{status}]")
PY
)"
